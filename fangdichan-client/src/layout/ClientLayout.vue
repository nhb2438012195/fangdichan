    ...
    <div class="app-body">
      <div class="activity-bar">
        <div v-for="item in navItems" :key="item.path"
             ...
             @click="router.push(item.path)" :title="item.label">
          {{ item.icon }}
        </div>
      </div>
      <div class="main-area">
...
const minimize = () => { if (window.electronAPI) window.electronAPI.minimize() }
const toggleMaximize = () => { if (window.electronAPI) window.electronAPI.toggleMaximize() }
const close = () => { if (window.electronAPI) window.electronAPI.close() }
</script>
...
.window-controls button:hover { background: #505050; }
...
.nav-icon { padding: 8px; border-radius: 4px; cursor: pointer; font-size: 18px; color: #ccc; }
.nav-icon:hover { background: #505050; }
.nav-icon.active { background: #007acc; }
.main-area { flex: 1; background: #f5f5f5; overflow: auto; padding: 16px; }
